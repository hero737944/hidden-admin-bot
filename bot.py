...
def is_bot_admin(uid):
    db = load_db()
    return uid == OWNER_ID or uid in db["admins"] or uid in db.get("super_admins", [])
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_adm:
        txt += (
            "\n📋 *Mod Commands*\n"
            "_(Use in GC by replying to user)_\n\n"
    ...
    if is_sup:
        txt += (
            "\n👑 *Admin Controls*\n\n"
            "➕ `/addadmin` — Add admin _(reply)_\n"
            "⭐ `/addsuperadmin` — Add super admin _(reply)_\n"
            "➖ `/removeadmin` — Remove admin _(reply, owner only)_\n"
...
async def new_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        ...
        await update.message.reply_text(
            ...
            f"━━━━━━━━━━━━━━━━━\n"
            f"🔐 Verify yourself to start chatting.\n"
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  HELPER — delete command in group
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
async def silent_delete(update):
...
async def warn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await user.send_message("❌ *Reply to a user's message to warn them.*", parse_mode="Markdown")
        return

    target = update.message.reply_to_message.from_user
    db     = load_db()
    tid    = str(target.id)
    db["warns"].setdefault(tid, 0)
    db["warns"][tid] += 1
    count = db["warns"][tid]
    save_db(db)
...
async def mute(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await user.send_message("❌ *Reply to a user's message to mute them.*", parse_mode="Markdown")
        return
    ...
    try:
        ...
        await user.send_message(f"❌ Mute failed: `{e}`", parse_mode="Markdown")
...
async def unmute(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await user.send_message("❌ *Reply to a user's message to unmute them.*", parse_mode="Markdown")
        return
    ...
    try:
        ...
        await user.send_message(f"❌ Unmute failed: `{e}`", parse_mode="Markdown")
...
async def ban(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await user.send_message("❌ *Reply to a user's message to ban them.*", parse_mode="Markdown")
        return
    ...
    try:
        ...
        await user.send_message(f"❌ Ban failed: `{e}`", parse_mode="Markdown")
...
async def warns(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text("❌ Reply to a user to check their warns.")
        return
...
async def addadmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if target.id in db["admins"] or target.id in db.get("super_admins", []) or target.id == OWNER_ID:
        await update.message.reply_text(f"⚠️ *{target.first_name}* is already an admin!", parse_mode="Markdown")
        return
...
async def removeadmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if removed:
        ...
        await update.message.reply_text(f"⚠️ *{target.first_name}* was not an admin.", parse_mode="Markdown")
...
async def adminlist(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Send to DM only
    await user.send_message(txt, parse_mode="Markdown")
